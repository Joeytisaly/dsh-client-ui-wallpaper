...
import { injectBootWallpaper } from "./boot-wallpaper.js";
import { WALLPAPER_MAX_BYTES, WALLPAPER_SETTINGS_NAMESPACE, WALLPAPER_URL, WallpaperSettingsSchema, } from "./wallpaper-settings.js";
export { WALLPAPER_FIELD, WALLPAPER_MAX_BYTES, WALLPAPER_SETTINGS_NAMESPACE, WALLPAPER_URL, } from "./wallpaper-settings.js";
...
function readEnabled(ctx) {
    ...
    if (settings === undefined)
        return false;
    const section = settings.get(WALLPAPER_NAMESPACE);
...
async function readBody(req, cap) {
    ...
    for await (const chunk of req) {
        ...
        total += buffer.length;
        if (total > cap)
            throw new Error(`wallpaper body exceeds ${cap} bytes`);
        chunks.push(buffer);
    }
    ...
    ctx.inject(['webServer'], (httpCtx) => {
        ...
        const dispose = httpCtx.webServer.register({
            ...
            handler: async (req, res) => {
                ...
                if (method === 'GET')
                    return handleWallpaperGet(req, res, wallpaperPath());
                if (method === 'POST')
                    return handleWallpaperPost(req, res, wallpaperPath());
                if (method === 'DELETE')
                    return handleWallpaperDelete(req, res, wallpaperPath());
